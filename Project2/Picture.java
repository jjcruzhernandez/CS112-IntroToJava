...
public class Picture { //creates a new image by specifying a h and w or filename
	...
	public Pixel getPixel(int x, int y) { // reference Pixel objects
		int[] pixelArray = null;

		try {
			pixelArray = raster.getPixel(x, y, pixelArray); 
		} catch (Exception e) {
			System.out.println("("+x+", "+y+") out of bounds.GET");
			System.exit(1);
		}
	...
	public void setPixel(int x, int y, Pixel pixel) { // reference Pixel objects
		int[] pixelArray = pixel.getComponents();

		try {
			raster.setPixel(x, y, pixelArray);
		} catch (Exception e) {
			System.out.println("("+x+", "+y+") out of bounds.SET");
			System.exit(1);
		}
	}
